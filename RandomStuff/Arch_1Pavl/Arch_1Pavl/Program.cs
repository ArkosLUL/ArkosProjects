...
namespace Arch_lab1
{
   ...
   //     static string[] extentions = new string[] { ".apt", ".err", ".log", ".pwi", ".sub", ".ttf", ".text", ".txt" };
        ...
        {
            string info;
            info = "Эта утилита меняет дату создания на текущую дату.\n";
        ...
        {
            string help;
            help = "Меняет дату создания на текущее время.\n\n";
            help += "CHANGEDATE [диск:][путь][имя файла] [/S] [/A[[:]атрибуты]] [[файл]...] \n\n";
            ...
            help += "Атрибуты можно установить через переменную среды CHANGEDATECMD.\n";
        ...
        static void readInputZero(string[] args)
        {
            while (true)
            {
                ...
                if (strInput.StartsWith("CHANGEDATE"))
                {
        ...
        static void EnvironmentVariable()//работаем с переменной окружения
        {
            ...
            if (environment["CHANGEDATECMD"] as string == "/A:A")
            {
                yesAttributes |= FileAttributes.Archive;
            }
            else if (environment["CHANGEDATECMD"] as string == "/A:-A")
            {
                noAttributes |= FileAttributes.Archive;
            }
            else if (environment["CHANGEDATECMD"] as string == "/A:H")
            {
                yesAttributes |= FileAttributes.Hidden;
            }
            else if (environment["CHANGEDATECMD"] as string == "/A:-H")
            ...
            else if (environment["CHANGEDATECMD"] as string == "/A:S")
            {
                yesAttributes |= FileAttributes.System;
            }
            else if (environment["CHANGEDATECMD"] as string == "/A:-S")
            ...
            else if (environment["CHANGEDATECMD"] as string == "/A:R")
            {
                yesAttributes |= FileAttributes.ReadOnly;
            }
            else if (environment["CHANGEDATECMD"] as string == "/A:-R")
               ...
               /*     bool flag = false;
                  ...
                  /*  if (!flag)
                    ...
                    try
                    {
                       ...
                       //  File.SetCreationTime(file, new DateTime(1917, 10, 8));
