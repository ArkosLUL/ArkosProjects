...
//ПОСТАНОВКА ЗАДАЧИ:
//       В первую строку текстового файла вписать его имя и расширение.
//ВЫПОЛНИЛА: 
...
namespace Arch_lab1
{
    class Program
    {
        ...
        {
            string help;
            help = "В первую строку текстового файла запись его имени и расширения.\n\n";
            help += "PUTNAME [диск:][путь][имя файла] [/S] [/A[[:]атрибуты]] [[файл]...] \n\n";
            ...
            help += "Атрибуты можно установить через переменную среды PUTNAMECMD.\n";
        ...
        static void readInputZero(string[] args)
        {
            while (true)
            {
                ...
                if (strInput.StartsWith("PUTNAME"))
                {
        ...
        static void readInputCommand(string[] args)
        {
            ...
            else
            {
                if (path == "")
                {
                    string str = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().GetName().CodeBase);
        ...
        static void EnvironmentVariable()//работаем с переменной окружения
        {
            ...
            if (environment["PUTNAMECMD"] as string == "/A:A")
            {
                yesAttributes |= FileAttributes.Archive;
            }
            else if (environment["PUTNAMECMD"] as string == "/A:-A")
            {
                noAttributes |= FileAttributes.Archive;
            }
            else if (environment["PUTNAMECMD"] as string == "/A:H")
            {
                yesAttributes |= FileAttributes.Hidden;
            }
            else if (environment["PUTNAMECMD"] as string == "/A:-H")
            ...
            else if (environment["PUTNAMECMD"] as string == "/A:S")
            {
                yesAttributes |= FileAttributes.System;
            }
            else if (environment["PUTNAMECMD"] as string == "/A:-S")
            ...
            else if (environment["PUTNAMECMD"] as string == "/A:R")
            {
                yesAttributes |= FileAttributes.ReadOnly;
            }
            else if (environment["PUTNAMECMD"] as string == "/A:-R")
